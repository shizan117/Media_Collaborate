                ...
                <div class="card card-primary">
                    <div class="card-body">
                        <div class="post-header">
                            <h1>Upload Edited Files</h1>
                            <a href="{{ route('video_editor.index') }}" class="btn btn-primary">Back to Posts</a>
                        </div>

                        <form id="post-form" action="{{ route('video_editor.update', $videoManPost->id) }}" method="POST" enctype="multipart/form-data">
                            @csrf
                            @method('PUT')

                            <div class="form-group">
                                <label for="content">Content:</label>
                                <p >{{ $videoManPost->content }}</p>
                            </div>
                            ...
                            <div class="form-group">
                                <strong>Link: </strong>
                                @if($videoManPost->link)
                                    <a href="{{ $videoManPost->link }}" class="btn btn-primary" target="_blank">{{ $videoManPost->link }}</a>
                                @else
                                    No Link
                                @endif </div>
...
<div class="card-footer text-center">
    ...
    <script>
        ...
        function removePreview(button, filePath) {
            ...
            fetch('{{ route('video_man_posts.removeFile') }}', {
                method: 'POST',
                headers: {
